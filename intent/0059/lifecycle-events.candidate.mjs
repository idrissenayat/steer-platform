...
export function correctedLifecycleEventDecision(serialized) {
  try {
    ...
    for (const bytes of inputs) {
      ...
      if (at === null || (prior !== null && at <= prior)) return blocked('EVENT_ORDER_INVALID');
      ...
      if (ids.has(event.eventId) || digests.has(event.recordDigest) || proofIds.has(proof.providerRecordId) || proofDigests.has(proof.recordDigest)) return blocked('EVENT_REPLAY');
      ids.add(event.eventId); digests.add(event.recordDigest); proofIds.add(proof.providerRecordId); proofDigests.add(proof.recordDigest);
      prior = at; current = event;
    }
