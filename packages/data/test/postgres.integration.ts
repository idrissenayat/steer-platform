...
try {
  ...
  if(selection.mode==='journey-performance'){
    ...
    console.log(`FOCUSED authenticated proposal continuation result: ${passed-1} joined check passed plus idempotent migration check; full suite NOT RUN.`);
  }else if(selection.mode==='journey-runtime-amendment'){
    ...
    assert.equal(passed,2);
    console.log(`FOCUSED authenticated new-linked result: ${passed-1} joined check passed plus idempotent migration check; full suite NOT RUN.`);
  }else if(selection.mode==='journey-runtime-revision'){
    console.log('FOCUSED authenticated existing pre-pull candidate revision through native corpus, recorded roles, confirmation and fixed save/reopen; NOT the full integration suite.');
  ...
  }else if(selection.mode==='owned-records-readset'||selection.mode==='owned-content-readset'||selection.mode==='owned-history-readset'){
    console.log(`FOCUSED ${selection.mode} with native corpus and crypto/SDK comparison oracle; NOT factory installation or C22.`);
    await testAuthenticatedGeneration({admin,connect,check},'new-distinct',false,false,true,selection.mode==='owned-history-readset'?'owned-history':selection.mode==='owned-content-readset'?'owned-content':'owned-records');
    assert.equal(passed,2);
    ...
    assert.equal(passed,2);
    console.log('FOCUSED native corpus graph: preceding joined journey, native records/source assertions and idempotent migrations passed; full suite NOT RUN.');
