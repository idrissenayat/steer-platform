...
import { createIntentDraftDiscovery } from '../src/intent-draft-discovery.ts';
import { discoveryFixture } from '../../tool-registry/test/intent-draft-discovery.fixture.ts';
...
test('discovery requires current metadata authority and exact scope before connecting', async () => {
  ...
  const service = createIntentDraftDiscovery(pool, config, { authorize: async () => { throw new Error('PRIVATE'); }, authorizeEntry: async () => {} });
...
test('timed-out discovery retains admission until dependencies settle and late leases are closed without reading content', async t => {
  ...
  const service = createIntentDraftDiscovery(pool, config, { authorize: async () => {}, authorizeEntry: async () => {} });
