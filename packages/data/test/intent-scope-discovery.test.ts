...
import { createIntentScopeDiscovery } from '../src/intent-scope-discovery.ts';
import { scopeDiscoveryFixture } from '../../tool-registry/test/intent-scope-discovery.fixture.ts';

const { input } = scopeDiscoveryFixture();
...
test('discovery requires current metadata authority and exact scope before connecting', async () => {
  ...
  const service = createIntentScopeDiscovery(pool, config, { authorize: async () => { throw new Error('PRIVATE'); }, authorizeEntry: async () => {} });
...
test('timed-out discovery retains admission until dependencies settle and late leases are closed without reading content', async t => {
  ...
  const service = createIntentScopeDiscovery(pool, config, { authorize: async () => {}, authorizeEntry: async () => {} });
