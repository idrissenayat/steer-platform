...
export function parseIntegrationSelection(args:readonly string[]) {
  ...
  if(args.length===1&&args[0]==='--local-records-upgrade')return {mode:'local-records-upgrade' as const};
  ...
  if(args.length===1&&args[0]==='--original-preservation')return {mode:'original-preservation' as const};
  ...
  if(args.length===1&&args[0]==='--combined-readset-feasibility')return {mode:'combined-readset-feasibility' as const};
  ...
  if(args.length===1&&args[0]==='--owned-history-readset')return {mode:'owned-history-readset' as const};
