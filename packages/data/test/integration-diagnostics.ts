...
export function parseIntegrationSelection(args:readonly string[]) {
  ...
  if(![2,4].includes(args.length)||args[0]!=='--clarification-repro'||!/^([1-9]|1[0-9]|20)$/.test(args[1]!)
    ...
    throw new Error('Use no arguments for the full suite, --scope-runtime, --development-history, --run-discovery, --admission-discovery, --candidate-save, --candidate-start, --candidate-journey, --journey-runtime, or --clarification-repro 1-20 [--query-delay-ms 0-5].');
