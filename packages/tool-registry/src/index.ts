...
import { intentAdmissionInputSchema, intentAdmissionOutputSchema, verifyIntentAdmissionDiscovery,
  type IntentAdmissionDiscovery, type IntentAdmissionOutput } from './intent-admission-discovery-contracts.ts';
import { candidateProposalInputSchema, candidateProposalOutputSchema, candidateProposalScopeSchema, verifyCandidateProposals,
  type CandidateProposalReader, type CandidateProposalOutput } from './candidate-proposal-contracts.ts';
import { candidateSaveStartInputSchema, candidateSaveStartOutputSchema, verifyCandidateSaveStart,
  type CandidateSaveStarter, type CandidateSaveStartOutput } from './candidate-save-start-contracts.ts';
import { candidateSaveStatusInputSchema, candidateSaveStatusOutputSchema, candidateSaveStatusScopeSchema, verifyCandidateSaveStatus,
  type CandidateSaveStatusReader, type CandidateSaveStatusOutput } from './candidate-save-status-contracts.ts';
import { candidateBundleReadInputSchema, candidateBundleReadOutputSchema, candidateBundleReadScopeSchema, verifyCandidateBundleRead,
  type CandidateBundleReadService, type CandidateBundleReadOutput } from './candidate-bundle-read-contracts.ts';
import { intentRunDiscoveryInputSchema, intentRunDiscoveryOutputSchema, type IntentRunDiscoveryReader, type IntentRunDiscoveryOutput } from './intent-run-discovery-contracts.ts';
...
import { z } from 'zod';
import { intentScopeHistoryInputSchema, intentScopeHistoryOutputSchema, verifyIntentScopeHistoryOutput,
  type IntentScopeHistoryReader, type IntentScopeHistoryOutput } from './intent-scope-history-contracts.ts';
import { candidateSavePreviewInputSchema, candidateSavePreviewOutputSchema, verifyCandidateSavePreview,
  type CandidateSavePreviewer, type CandidateSavePreviewOutput } from './candidate-save-preview-contracts.ts';
import { candidateSavePrepareInputSchema, candidateSavePrepareOutputSchema, verifyCandidateSavePrepare,
  type CandidateSavePreparer, type CandidateSavePrepareOutput } from './candidate-save-prepare-contracts.ts';
import { candidateSaveReviewInputSchema, candidateSaveReviewOutputSchema, verifyCandidateSaveReview,
  type CandidateSaveReviewer, type CandidateSaveReviewOutput } from './candidate-save-review-contracts.ts';
...
const runDiscoveryGuard = defineQuery({ name: 'intent.runs.discover',
  description: 'Find retained scope and development references across preserved owner draft revisions. Metadata only, never completed work or permission to execute.',
  input: intentRunDiscoveryInputSchema, output: principalSchema, handler: (_input, principal) => principal });
const runDiscovery = Object.freeze({ ...runDiscoveryGuard, output: intentRunDiscoveryOutputSchema,
  async invoke(raw: unknown, context: InvocationContext): Promise<IntentRunDiscoveryOutput> {
    const initial = runDiscoveryGuard.invoke(raw, context), input = intentRunDiscoveryInputSchema.parse(raw);
    if (initial.type !== 'human') throw new ToolError('FORBIDDEN');
    const service = context.services?.intentRunDiscovery;
    if (!service || !context.revalidate) throw new ToolError('UNAVAILABLE');
    const revalidate = async () => {
      await freshToolPrincipal(runDiscoveryGuard, input, initial, context);
      if (service.scope.subject !== initial.subject || (['organizationId', 'productId', 'repository'] as const).some(k => service.scope[k] !== input[k])) throw new ToolError('FORBIDDEN');
    };
    await revalidate(); let rawOutput: unknown, failed = false;
    ...
    await revalidate(); if (failed) throw new ToolError('UNAVAILABLE');
    try {
      const output = intentRunDiscoveryOutputSchema.parse(rawOutput);
      if ((Object.keys(input) as Array<keyof typeof input>).some(k => JSON.stringify(input[k]) !== JSON.stringify(output[k]))) throw new Error();
      await revalidate(); return output;
    } catch (error) { if (error instanceof ToolError) throw error; throw new ToolError('INTERNAL_ERROR'); }
  },
});
...
const scopeHistoryGuard = defineQuery({ name: 'intent.scope.history',
  description: 'Inspect historical scope findings under current records authority; never grant current clearance, retry, save or sign.',
  input: intentScopeHistoryInputSchema, output: principalSchema, handler: (_input, principal) => principal });
const scopeHistory = Object.freeze({ ...scopeHistoryGuard, output: intentScopeHistoryOutputSchema,
  async invoke(raw: unknown, context: InvocationContext): Promise<IntentScopeHistoryOutput> {
    const initial = scopeHistoryGuard.invoke(raw, context), input = intentScopeHistoryInputSchema.parse(raw);
    if (initial.type !== 'human') throw new ToolError('FORBIDDEN');
    const service = context.services?.intentScopeHistoryReader;
    if (!service || !context.revalidate) throw new ToolError('UNAVAILABLE');
    const revalidate = async () => {
      await freshToolPrincipal(scopeHistoryGuard, input, initial, context);
      if (service.scope.subject !== initial.subject || (['organizationId', 'productId', 'repository'] as const).some(k => service.scope[k] !== input[k])) throw new ToolError('FORBIDDEN');
    };
    await revalidate(); let rawOutput: unknown, failed = false;
    ...
    await revalidate(); if (failed) throw new ToolError('UNAVAILABLE');
    try {
      const output = await verifyIntentScopeHistoryOutput(rawOutput);
...
const candidateStartGuard = defineQuery({ name: 'intent.candidate.save.start',
  description: 'Explicitly request the same preserved candidate save workflow under current authority. An acknowledgement is not a verified Git save or a gate signature.',
  input: candidateSaveStartInputSchema, output: principalSchema, handler: (_input, principal) => principal });
const candidateStart = Object.freeze({ ...candidateStartGuard, kind: 'command' as const, output: candidateSaveStartOutputSchema,
  async invoke(raw: unknown, context: InvocationContext): Promise<CandidateSaveStartOutput> {
    const initial = candidateStartGuard.invoke(raw, context), input = candidateSaveStartInputSchema.parse(raw);
    if (initial.type !== 'human') throw new ToolError('FORBIDDEN');
    const service = context.services?.candidateSaveStarter;
    if (!service || !context.revalidate) throw new ToolError('UNAVAILABLE');
    const revalidate = async () => {
      await freshToolPrincipal(candidateStartGuard, input, initial, context);
      if (service.scope.subject !== initial.subject || (['organizationId', 'productId', 'repository', 'branch'] as const).some(k => service.scope[k] !== input[k])) throw new ToolError('FORBIDDEN');
    };
    await revalidate(); let result: unknown, failed = false;
    try { result = await service.start(input, revalidate); } catch { failed = true; }
    await revalidate(); if (failed) throw new ToolError('UNAVAILABLE');
    let output: CandidateSaveStartOutput;
    try { output = verifyCandidateSaveStart(input, result); } catch { throw new ToolError('INTERNAL_ERROR'); }
    await revalidate(); return output;
  },
});

const candidatePrepareGuard = defineQuery({ name: 'intent.candidate.save.prepare',
  description: 'Confirm the exact current package and preserve its immutable original. Does not start a workflow, save Git, or sign a gate.',
  input: candidateSavePrepareInputSchema, output: principalSchema, handler: (_input, principal) => principal });
const candidatePrepare = Object.freeze({ ...candidatePrepareGuard, kind: 'command' as const, output: candidateSavePrepareOutputSchema,
  async invoke(raw: unknown, context: InvocationContext): Promise<CandidateSavePrepareOutput> {
    const initial = candidatePrepareGuard.invoke(raw, context), input = candidateSavePrepareInputSchema.parse(raw);
    if (initial.type !== 'human' || input.confirmation.subject !== initial.subject) throw new ToolError('FORBIDDEN');
    const service = context.services?.candidateSavePreparer;
    if (!service || !context.revalidate) throw new ToolError('UNAVAILABLE');
    const revalidate = async () => {
      await freshToolPrincipal(candidatePrepareGuard, input, initial, context);
      if (service.scope.subject !== initial.subject || service.scope.branch !== input.confirmation.branch
        || (['organizationId', 'productId', 'repository', 'configurationRevision'] as const).some(k => service.scope[k] !== input.preview[k])) throw new ToolError('FORBIDDEN');
    };
    await revalidate(); let result: unknown, failed = false;
    try { result = await service.prepare(input, revalidate); } catch { failed = true; }
    await revalidate(); if (failed) throw new ToolError('UNAVAILABLE');
    let output: CandidateSavePrepareOutput;
    try { output = verifyCandidateSavePrepare(input, result); } catch { throw new ToolError('INTERNAL_ERROR'); }
    await revalidate(); return output;
  },
});

const candidatePreviewGuard = defineQuery({ name: 'intent.candidate.save.preview',
  description: 'Preview an exact candidate package with retained agent lineage and current destination evidence. No confirmation, admission or saving.',
  input: candidateSavePreviewInputSchema, output: principalSchema, handler: (_input, principal) => principal });
const candidatePreview = Object.freeze({ ...candidatePreviewGuard, output: candidateSavePreviewOutputSchema,
  async invoke(raw: unknown, context: InvocationContext): Promise<CandidateSavePreviewOutput> {
    const initial = candidatePreviewGuard.invoke(raw, context), input = candidateSavePreviewInputSchema.parse(raw);
    if (initial.type !== 'human') throw new ToolError('FORBIDDEN');
    const service = context.services?.candidateSavePreviewer;
    if (!service || !context.revalidate) throw new ToolError('UNAVAILABLE');
    const revalidate = async () => {
      await freshToolPrincipal(candidatePreviewGuard, input, initial, context);
      if (service.scope.subject !== initial.subject || (['organizationId', 'productId', 'repository', 'configurationRevision'] as const).some(k => service.scope[k] !== input[k])) throw new ToolError('FORBIDDEN');
    };
    await revalidate(); let result: unknown, failed = false;
    try { result = await service.preview(input, revalidate); } catch { failed = true; }
    await revalidate(); if (failed) throw new ToolError('UNAVAILABLE');
    let output: CandidateSavePreviewOutput;
    try { output = await verifyCandidateSavePreview(input, result);
      if (output.review.subject !== initial.subject || output.review.branch !== service.scope.branch) throw new Error();
    } catch { throw new ToolError('INTERNAL_ERROR'); }
    await revalidate(); return output;
  },
});

const candidateSaveReviewGuard = defineQuery({ name: 'intent.candidate.save.review',
  description: 'Recheck the exact preserved Brief, Spec, Exam and human direction against current recorded scope. Read-only: no confirmation, admission, generation or saving.',
  input: candidateSaveReviewInputSchema, output: principalSchema, handler: (_input, principal) => principal });
const candidateSaveReview = Object.freeze({ ...candidateSaveReviewGuard, output: candidateSaveReviewOutputSchema,
  async invoke(raw: unknown, context: InvocationContext): Promise<CandidateSaveReviewOutput> {
    const initial = candidateSaveReviewGuard.invoke(raw, context), input = candidateSaveReviewInputSchema.parse(raw);
    if (initial.type !== 'human') throw new ToolError('FORBIDDEN');
    const service = context.services?.candidateSaveReviewer;
    if (!service || !context.revalidate) throw new ToolError('UNAVAILABLE');
    const revalidate = async () => {
      await freshToolPrincipal(candidateSaveReviewGuard, input, initial, context);
      if (service.scope.subject !== initial.subject || (['organizationId', 'productId', 'repository', 'configurationRevision'] as const).some(k => service.scope[k] !== input[k])) throw new ToolError('FORBIDDEN');
    };
    await revalidate(); let result: unknown, failed = false;
    try { result = await service.review(input, revalidate); } catch { failed = true; }
    await revalidate(); if (failed) throw new ToolError('UNAVAILABLE');
    let output: CandidateSaveReviewOutput;
    try { output = await verifyCandidateSaveReview(input, result);
      if (output.subject !== initial.subject || output.branch !== service.scope.branch) throw new Error();
    } catch { throw new ToolError('INTERNAL_ERROR'); }
    await revalidate(); return output;
  },
});

const candidateStatusGuard = defineQuery({ name: 'intent.candidate.save.status',
  description: 'Inspect the original candidate save receipt without writing or retrying. Missing or unknown receipts never authorize another submission.',
  input: candidateSaveStatusInputSchema, output: principalSchema, handler: (_input, principal) => principal });
const candidateStatus = Object.freeze({ ...candidateStatusGuard, output: candidateSaveStatusOutputSchema,
  async invoke(raw: unknown, context: InvocationContext): Promise<CandidateSaveStatusOutput> {
    const initial = candidateStatusGuard.invoke(raw, context), input = candidateSaveStatusInputSchema.parse(raw);
    const service = context.services?.candidateSaveStatusReader;
    if (!service || !context.revalidate) throw new ToolError('UNAVAILABLE');
    const revalidate = async () => {
      await freshToolPrincipal(candidateStatusGuard, input, initial, context);
      const parsed = candidateSaveStatusScopeSchema.safeParse(service.scope);
      if (!parsed.success || parsed.data.subject !== initial.subject
        || (['organizationId', 'productId', 'repository', 'branch'] as const).some(key => parsed.data[key] !== input[key])) throw new ToolError('FORBIDDEN');
    };
    await revalidate(); let result: unknown, failed = false;
    try { result = await service.read(input, revalidate); } catch { failed = true; }
    await revalidate(); if (failed) throw new ToolError('UNAVAILABLE');
    let output: CandidateSaveStatusOutput;
    try {
      output = verifyCandidateSaveStatus(input, result);
      if (output.outcome === 'committed' && !service.scope.itemIds.includes(output.reference.itemId)) throw new Error();
    } catch { throw new ToolError('INTERNAL_ERROR'); }
    await revalidate(); return output;
  },
});

const candidateReadGuard = defineQuery({ name: 'intent.candidate.read',
  description: 'Reopen all candidate documents at an exact Git commit and manifest digest under current read authority. No latest fallback, writes, scope acceptance or gate approval.',
  input: candidateBundleReadInputSchema, output: principalSchema, handler: (_input, principal) => principal });
const candidateRead = Object.freeze({ ...candidateReadGuard, output: candidateBundleReadOutputSchema,
  async invoke(raw: unknown, context: InvocationContext): Promise<CandidateBundleReadOutput> {
    const initial = candidateReadGuard.invoke(raw, context), input = candidateBundleReadInputSchema.parse(raw);
    const service = context.services?.candidateBundleReader;
    if (!service || !context.revalidate) throw new ToolError('UNAVAILABLE');
    const revalidate = async () => {
      await freshToolPrincipal(candidateReadGuard, input, initial, context);
      const parsed = candidateBundleReadScopeSchema.safeParse(service.scope);
      if (!parsed.success || parsed.data.subject !== initial.subject || !parsed.data.itemIds.includes(input.itemId)
        || (['organizationId', 'productId', 'repository', 'branch'] as const).some(key => parsed.data[key] !== input[key])) throw new ToolError('FORBIDDEN');
    };
    await revalidate(); let result: unknown, failed = false;
    try { result = await service.read(input, revalidate); } catch { failed = true; }
    await revalidate(); if (failed) throw new ToolError('UNAVAILABLE');
    let output: CandidateBundleReadOutput;
    try { output = await verifyCandidateBundleRead(input, result); } catch { throw new ToolError('INTERNAL_ERROR'); }
    await revalidate(); return output;
  },
});

const candidateProposalsGuard = defineQuery({ name: 'intent.candidate.proposals',
  description: 'List verified proposal pointers at an exact reviewed Git commit. No lifecycle, editability, newness or save authority is inferred.',
  input: candidateProposalInputSchema, output: principalSchema, handler: (_input, principal) => principal });
const candidateProposals = Object.freeze({ ...candidateProposalsGuard, output: candidateProposalOutputSchema,
  async invoke(raw: unknown, context: InvocationContext): Promise<CandidateProposalOutput> {
    const initial = candidateProposalsGuard.invoke(raw, context), input = candidateProposalInputSchema.parse(raw);
    const service = context.services?.candidateProposalReader;
    if (!service || !context.revalidate) throw new ToolError('UNAVAILABLE');
    const current = async () => {
      await freshToolPrincipal(candidateProposalsGuard, input, initial, context);
      const scope = candidateProposalScopeSchema.safeParse(service.scope);
      if (!scope.success || scope.data.subject !== initial.subject || !scope.data.itemIds.includes(input.itemId)
        || (['organizationId', 'productId', 'repository', 'branch'] as const).some(k => scope.data[k] !== input[k])) throw new ToolError('FORBIDDEN');
    };
    await current(); let result: unknown, failed = false;
    try { result = await service.list(input, current); } catch { failed = true; }
    await current(); if (failed) throw new ToolError('UNAVAILABLE');
    let output: CandidateProposalOutput;
    try { output = verifyCandidateProposals(input, result); } catch { throw new ToolError('INTERNAL_ERROR'); }
    await current(); return output;
  },
});

const admissionDiscoveryGuard = defineQuery({ name: 'intent.admissions.discover',
  description: 'Read configured preparation metadata and whether an original input record was observed. No content recovery, execution-state claim or retry authority.',
  input: intentAdmissionInputSchema, output: principalSchema, handler: (_input, principal) => principal });
const admissionDiscovery = Object.freeze({ ...admissionDiscoveryGuard, output: intentAdmissionOutputSchema,
  async invoke(raw: unknown, context: InvocationContext): Promise<IntentAdmissionOutput> {
    const initial = admissionDiscoveryGuard.invoke(raw, context), input = intentAdmissionInputSchema.parse(raw);
    const service = context.services?.intentAdmissionDiscovery;
    if (!service || !context.revalidate) throw new ToolError('UNAVAILABLE');
    const current = async () => {
      await freshToolPrincipal(admissionDiscoveryGuard, input, initial, context);
      if (service.scope.subject !== initial.subject || (['organizationId', 'productId', 'repository'] as const).some(k => service.scope[k] !== input[k])) throw new ToolError('FORBIDDEN');
    };
    await current(); let result: unknown, failed = false;
    try { result = await service.discover(input, current); } catch { failed = true; }
    await current(); if (failed) throw new ToolError('UNAVAILABLE');
    let output: IntentAdmissionOutput;
    try { output = verifyIntentAdmissionDiscovery(input, result); } catch { throw new ToolError('INTERNAL_ERROR'); }
    await current(); return output;
  },
});

// Frozen definitions are the common source for discovery, dispatch and HTTP contracts.
const definitions = Object.freeze([admissionDiscovery, candidateProposals, candidateStart, candidatePrepare, candidatePreview, runDiscovery, developmentHistory, scopeHistory, candidateSaveReview, candidateStatus, candidateRead, scopeDiscovery, scopeStart, scopePrepare, scopeRead, draftDiscovery, developmentReview, developmentPrepare, developmentStart, developmentRead,draftCreate, draftAppend, draftRead, Object.freeze(overlapQuery), Object.freeze(agentCommand), Object.freeze(contextQuery), Object.freeze(projectionQuery), Object.freeze(reconciliationStart), Object.freeze(reconciliationStatus), Object.freeze(recordedBriefStart), Object.freeze(recordedBriefStatus), Object.freeze(recordedBriefRecover), Object.freeze(recordedBriefRecoveryStatus), Object.freeze(changesQuery), Object.freeze(snapshotQuery), Object.freeze(briefQuery), Object.freeze(artifactsQuery), Object.freeze(decisionsQuery), Object.freeze(evidenceQuery), Object.freeze(catalogQuery), Object.freeze(previewQuery), Object.freeze(briefSaveCommand), Object.freeze(briefSaveStatusQuery), Object.freeze(destinationQuery)]);
export function invokeTool(name: 'intent.overlap.check', input: unknown, context: InvocationContext): Promise<IntentOverlapOutput>;
export function invokeTool(name: 'intent.admissions.discover', input: unknown, context: InvocationContext): Promise<IntentAdmissionOutput>;
export function invokeTool(name: 'intent.candidate.proposals', input: unknown, context: InvocationContext): Promise<CandidateProposalOutput>;
...
export function invokeTool(name: 'intent.candidate.save.review', input: unknown, context: InvocationContext): Promise<CandidateSaveReviewOutput>;
export function invokeTool(name: 'intent.candidate.save.prepare', input: unknown, context: InvocationContext): Promise<CandidateSavePrepareOutput>;
export function invokeTool(name: 'intent.candidate.save.start', input: unknown, context: InvocationContext): Promise<CandidateSaveStartOutput>;
export function invokeTool(name: 'intent.candidate.save.preview', input: unknown, context: InvocationContext): Promise<CandidateSavePreviewOutput>;
...
export function invokeTool(name: 'intent.scope.history', input: unknown, context: InvocationContext): Promise<IntentScopeHistoryOutput>;
...
export function invokeTool(name: 'intent.runs.discover', input: unknown, context: InvocationContext): Promise<IntentRunDiscoveryOutput>;
