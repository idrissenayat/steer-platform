...
const scope = { organizationId: identifier, repository: identifier,
  domain: gatePolicyInputSchema.shape.policy.shape.activatedDomains.element,
  reviewerSubject: identifier, configurationRevision: identifier };
const bindings = z.strictObject({ ...scope, recordItem: identifier,
  ...
  builderSubject: identifier, executionId: identifier, builderExecutionId: identifier, reviewedAt: instant });
const expectedSchema = bindings.extend({ proofDigest: digest });
const trustSchema = z.strictObject({ version: z.literal('steer-domain-review-runner-trust/v1'), ...scope,
...
const payloadSchema = z.strictObject({ version: z.literal('steer-domain-review-runner-attestation/v1'),
...
const envelopeSchema = z.strictObject({ version: z.literal('steer-domain-review-runner-proof/v1'),
  payload: z.string().min(1).max(16384), signatureBase64: z.string().length(88).regex(/^[A-Za-z0-9+/]{86}==$/) });
const hash = (value: string) => createHash('sha256').update(value).digest('hex');

/** Verify a selected runner's signed assertion about one exact native review.
 * This is a development receipt contract, not an installed provider integration
 * or independent proof that the runner's isolation claims are truthful. */
export function verifyDomainReviewRunnerAttestation(rawEnvelope: unknown, rawTrust: unknown, rawExpected: unknown, evaluatedAt: unknown) {
  try {
    ...
    if (JSON.stringify(payload) !== envelope.payload ||
      Object.keys(scope).some(key => payload[key as keyof typeof payload] !== trust[key as keyof typeof trust]) ||
      payload.attestor !== trust.attestor || payload.keyId !== trust.keyId) return null;
    // Compare against external selection, never against the receipt's own claims.
    for (const key of Object.keys(bindings.shape)) if (payload[key as keyof typeof payload] !== expected[key as keyof typeof expected]) return null;
    if (payload.executionId === payload.builderExecutionId || payload.reviewerSubject === payload.builderSubject) return null;
    ...
    if (!verify(null, Buffer.concat([Buffer.from('steer-domain-review-runner-attestation/v1\0'), bytes]), key, signature)) return null;
    return Object.freeze({ kind: 'verified-domain-review-runner-attestation' as const,
      claims: Object.freeze(payload), trustDigest: hash(JSON.stringify(trust)), proofDigest: expected.proofDigest, evaluatedAt: evaluation,
