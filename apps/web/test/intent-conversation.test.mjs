...
test('real conversation component sends free text, follows up and displays three inert candidates without saving', async () => {
  ...
  const runHistory = await compile('intent-run-history', { './intent-admission-discovery-transport': new URL('../app/intent-admission-discovery-transport.ts', import.meta.url).href, './intent-development-history': history, './intent-scope-panel': scopePanel, './intent-run-discovery-transport': new URL('../app/intent-run-discovery-transport.ts', import.meta.url).href });
