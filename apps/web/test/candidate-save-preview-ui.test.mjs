...
async function fixture() {
  ...
  const state = { calls: [], output: f.prepared.output, page, denied: false, wait: null };
...
test('actual package component selects a retained run and previews exact documents without confirmation, regeneration or save', async () => {
  ...
  try {
    ...
    assert.match(document.body.textContent, /All document bytes match/); assert.match(document.body.textContent, /Durable human confirmation and save start are not enabled/);
    assert.equal(t.state.calls.length, 2); assert.equal(document.activeElement.textContent, 'Exact package preview — not saved');
    assert.equal([...document.querySelectorAll('button')].some(b => /confirm|save to/i.test(b.textContent)), false);
