...
export class AgentScopeChangedError extends Error {
  constructor() { super('Existing scope changed. Check current sources and confirm your direction again. Your text is still here; no GitHub save occurred.'); }
}
  ...
  return {
    ...
    async develop(input: AgentInput) {
      ...
      try {
        ...
        if (response.status === 503) throw new Error('The agent could not complete this request. Its connection or model budget may be unavailable. Your text is still here; no GitHub save occurred.');
