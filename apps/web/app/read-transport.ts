const tools = ['intent.candidate.proposals', 'intent.candidate.save.preview', 'intent.candidate.save.review', 'intent.candidate.save.status', 'intent.candidate.read', 'intent.overlap.check', 'projection.snapshot.read', 'projection.changes.read', 'intent.brief.catalog', 'intent.brief.read', 'intent.brief.artifacts', 'intent.brief.decisions', 'intent.brief.decision.evidence', 'intent.brief.preview', 'intent.brief.destination', 'intent.brief.save.status'] as const;
...
export function createReadTransport(origin: string, transport: typeof fetch = globalThis.fetch) {
  ...
  const request = async (name: (typeof tools)[number], input: unknown): Promise<unknown> => {
    ...
    const timer = setTimeout(() => controller.abort(), ['intent.candidate.proposals', 'intent.candidate.save.review', 'intent.candidate.save.preview'].includes(name) ? 70000 : TIMEOUT_MS);
    const work = async () => {
