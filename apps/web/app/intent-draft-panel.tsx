...
import type { IntentDraftDiscoveryOutput, IntentDraftDiscoveryEntry } from '@steer/tool-registry/intent-draft-discovery-contracts';
...
export default function IntentDraftPanel({ organizationId, productId, repository, subject, expiresAt, content, locked, onRestore, enabled = false, onResult }: {
  ...
  useEffect(() => {
    ...
    const close = () => { closed = true; current.close(); search.close(); setDiscoveryPage(null); setDiscoveryError(''); setDiscoveredDraft(null); setView(current.snapshot()); setDraftId(''); };
    ...
    return () => { current.close(); search.close(); if (discovery.current === search) discovery.current = null; if (editor.current === current) editor.current = null; clearInterval(timer);
  ...
  async function findDrafts(cursor: IntentDraftDiscoveryOutput['cursor'] = null) {
    const current = discovery.current;
    if (!current || discovering.current || document.hidden || Date.now() >= Date.parse(expiresAt)) return;
    discovering.current = true; setDiscoveryBusy(true); setDiscoveryPage(null); setDiscoveredDraft(null); setDiscoveryError('');
    try {
      const page = await current.discover({ organizationId, productId, repository, cursor });
      if (discovery.current === current && !document.hidden && Date.now() < Date.parse(expiresAt)) setDiscoveryPage(page);
    } catch { if (discovery.current === current) setDiscoveryError('Your drafts could not be listed with current access. This is not an empty result. Your editor is unchanged.'); }
    finally { if (discovery.current === current) { discovering.current = false; setDiscoveryBusy(false); } }
  }
  ...
  return <section className="intent-draft-preservation access-note" aria-labelledby="draft-preservation-title">
    ...
    {view.draftId && <p>Draft reference: <code>{view.draftId}</code>. Use “Find my drafts” after signing in again, or keep this reference. A reference alone does not confirm preserved content.</p>}
    ...
    <section className="intent-draft-discovery" aria-labelledby="draft-discovery-title"><h4 id="draft-discovery-title">Your working drafts</h4>
      <p>Find drafts and retained runs after refresh. Nothing is loaded into the editor or restarted automatically.</p>
      <button className="access-secondary" type="button" disabled={discoveryBusy || locked || pending || view.retryAvailable} onClick={() => { void findDrafts(); }}>Find my drafts</button>
      {discoveryBusy && <p role="status">Finding currently accessible draft references…</p>}
      {discoveryError && <p role="alert">{discoveryError}</p>}
      {discoveryPage && <><p>Current owner and records configuration only. Document text is loaded only when you choose a draft.</p>
        {!discoveryPage.entries.length && <p role="status">No resumable references on this page. Expired, held, discarded, published or differently configured records are not listed. This does not check existing GitHub intents.</p>}
        <ul>{discoveryPage.entries.map(entry => <li key={entry.draftId} data-draft-reference={entry.draftId}>
          <p>Draft created <time dateTime={entry.createdAt}>{new Date(entry.createdAt).toLocaleString()}</time> · <code>{entry.draftId}</code></p>
          <p>{entry.latest ? `Stored revision ${entry.latest.revision} · ${entry.run ? 'retained run reference available' : 'no retained run listed for this revision'}` : 'Reference only — no preserved content listed'}</p>
