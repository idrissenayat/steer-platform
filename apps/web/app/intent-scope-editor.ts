...
const initial = (): ScopeEditorView => ({ status: 'idle', source: null, operation: null, preparation: null,
  observation: null, sourceInvalidated: false, locked: false, retryAvailable: false, message: '' });
...
export function createIntentScopeEditor(transport: IntentScopeTransport,
  ...
  let view = initial(), closed = false, busy = false, retry: 'prepare' | 'start' | null = null;
  ...
  async function read() {
    ...
    try {
      ...
      publish({ status: result.status, observation: result, message: '' });
  ...
  async function execute() {
    ...
    try {
      ...
      retry = null; publish({ status: 'pending', message: 'Review workflow acknowledged. Checking verified findings…' }); readAfter = true;
  ...
  return {
    ...
    async assess() {
      ...
      retry = 'prepare'; publish({ ...initial(), source: structuredClone({ ...source, input }) }); await execute();
    },
