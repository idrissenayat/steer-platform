...
export default function IntentConversation({ organizationId, subject, expiresAt, enabled, repository = null, draftProductId = null }: {
  ...
  async function send() {
    ...
    try {
      ...
      ? cause.message : 'The agent could not finish. Your text is still here. No GitHub save occurred.'); }
  ...
  function restoreDraft(content: IntentDraftContent) {
    if (!live.current || pending.current) return;
    setIntent(content.originalText); setClarification(''); setRestoredClarifications(content.clarificationTurns);
    setEditedDocuments(content.documents); setResult(null); setDisposition(null); setError('');
    setInvalidatedReviews(invalidateIntentReviews(['source'])); setDocumentRevision(1);
    setScopeReviewVersion(version => version + 1); setSelected('brief'); setDocumentMode('preview');
    setRestoreFocus(version => version + 1);
  }
    ...
    {expired ? <p role="alert">This conversation was cleared when the session expired or the page was hidden. <a href="/">Refresh access</a> to continue.</p> : <>
      ...
      {editedDocuments && <div className="intent-documents"><h3 tabIndex={-1} ref={documentHeading}>Your draft documents</h3>
        ...
        <details><summary>{result?.documents ? 'Original source and generation reference' : 'Restored source text'}</summary>
          ...
          {restoredClarifications.map((turn, index) => <pre className="intent-draft-source" key={index}>{turn}</pre>)}
          {result?.documents && <p className="access-hint">Generation reference: <code>{result.sourceDigest}</code>. This identifies the generated source context, not your edited documents or an approval.</p>}
        </details>
        ...
        <div className="intent-document-buttons" aria-label="Document viewing mode">{(['preview', 'edit', 'original'] as const).filter(mode => mode !== 'original' || result?.documents).map(mode =>
        ...
        <p role="status">{selected.toUpperCase()}.md · {result?.documents && editedDocuments[selected] === result.documents[selected] ? 'No edits yet' : 'Working draft — check preservation status'}</p>
        ...
        </> : <BriefMarkdown content={documentMode === 'original' && result?.documents ? result.documents[selected] : editedDocuments[selected]} />}
        {documentMode === 'original' && <p className="access-hint">Generated original · your edits remain unchanged in your draft.</p>}
        {(!result?.documents || (['brief', 'spec', 'exam'] as const).some(name => editedDocuments[name] !== result.documents![name])) &&
        ...
        <p className="access-hint">{result?.documents ? 'The Exam was drafted in a separate Test Agent context. ' : ''}These documents still need review and authorized repository saving.</p>
      </div>}
    </>}
  </section>;
}
