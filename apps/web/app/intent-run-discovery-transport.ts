import { intentRunDiscoveryInputSchema,intentRunDiscoveryOutputSchema,type IntentRunDiscoveryInput } from '@steer/tool-registry/intent-run-discovery-contracts';

const failure=()=>new Error('Retained agent runs are unavailable. Your editor is unchanged.');
...
export function createIntentRunDiscoveryTransport(origin:string,transport:typeof fetch=fetch){
  ...
  return{
    async discover(raw:IntentRunDiscoveryInput){
      if(closed||active)throw failure();const input=intentRunDiscoveryInputSchema.parse(raw),body=JSON.stringify(input);
      ...
      const work=(async()=>{
        try{
          response=await transport(`${origin}/v1/tools/intent.runs.discover`,{method:'POST',body,
          ...
          const value=await intentRunDiscoveryOutputSchema.parse(JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(joined)));
          controller.signal.throwIfAborted();
