import { candidateSavePrepareInputSchema, verifyCandidateSavePrepare, type CandidateSavePrepareInput } from '@steer/tool-registry/candidate-save-prepare-contracts';

const fail = () => new Error('Confirmation acknowledgement could not be verified. Recover the same confirmation; do not create another submission.');
...
export function createCandidateSavePrepareClient(origin: string, transport: typeof fetch = fetch) {
  ...
  return {
    async prepare(raw: CandidateSavePrepareInput) {
      const input = candidateSavePrepareInputSchema.parse(raw), body = JSON.stringify(input);
      ...
      try {
        const output = await Promise.race([interrupted, (async () => {
          response = await transport(`${origin}/v1/tools/intent.candidate.save.prepare`, { method: 'POST', body,
          ...
          return verifyCandidateSavePrepare(input, JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(joined)));
        })()]);
