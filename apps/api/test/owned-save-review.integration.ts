...
export async function testOwnedSaveReview(f: Awaited<ReturnType<typeof scopeDraftIntegrationFixture>>,
  ...
  const setup = (mode: string, finalKey?: () => Promise<void>) => {
    let scopeKeys = 0, callerAllowed = true, draftAllowed = true, sourceAllowed = true;
    ...
    const rawDrafts = createIntentDraftService(f.pools.drafts, f.config, { lifecycle: { authorize: current }, revisions: {
      authorize: async () => { await current(); if (!draftAllowed) throw new Error('PRIVATE draft revoked'); }, keyForDraft: f.deps.keyForDraft,
    } });
    ...
    const deps = { drafts: rawDrafts, sources, scopeReview: mode === 'wrapped' ? wrapped : scope, authorizeReview: current };
    const review = createRecordedCandidateSaveReviewer(f.config, deps);
    return { scope, rawDrafts, sources, review, current, binding, scopeKeys: () => scopeKeys,
  ...
  for (const mode of ['exact', 'wrapped', 'record', 'key', 'revision', 'caller', 'draft', 'source', 'method', 'rotated-key']) {
    const s = setup(mode);
    try {
      if (mode === 'exact' || mode === 'wrapped') {
        ...
        assert.equal(s.scopeKeys(), mode === 'exact' ? 2 : 4);
      } else await assert.rejects(s.review.review(input, s.current), /unavailable/, mode);
  ...
  console.log('PASS owned final-save assessment: two native scope-key reads instead of four, exact wrapped-reader output parity, eight final-key denial cases and held-scope drainage before source owner completion');
}
