...
const kind=(call:{path:string;method:string}):keyof Counts=>call.method==='POST'&&/^\/app\/installations\/[^/]+\/access_tokens$/.test(call.path)?'token'
...
export function createNativeRequestMeter(transport:typeof fetch){
  ...
  const measured:typeof fetch=(input,init)=>{
    ...
    try{category=kind({path:new URL(input instanceof Request?input.url:String(input)).pathname,
      method:init?.method??(input instanceof Request?input.method:'GET')});}catch{}
