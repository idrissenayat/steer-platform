...
export async function recordedRuntimeFixture(t: { after(run: () => void): void }) {
  const source = fixture(t), keys = await generateKeyPair('RS256'), app = await generateKeyPair('RS256', { extractable: true });
  ...
  const selection = { itemId: 'items/0176-recorded', idempotencyKey: '17600000-0000-4000-8000-000000000001' };
  ...
  const workflowId = `steer-recorded-brief/v1/org/github%3A52/items%2F0176-recorded/${selection.idempotencyKey}`;
