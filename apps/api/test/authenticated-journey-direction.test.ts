...
test('missing, ambiguous or noncandidate target cannot silently become new distinct work', async () => {
  ...
  assert.throws(() => authenticatedJourneyChoice('candidate-revision', f.evidence));
  target.status = 'candidate'; assert.throws(() => authenticatedJourneyChoice('candidate-revision', f.evidence));
  target.path = 'items/0002-existing/candidates/57762718-d38a-4926-b96d-7a1c40fdd6f7/BRIEF.md';
  assert.throws(() => authenticatedJourneyChoice('candidate-revision', { ...f.evidence, inventory: [...f.evidence.inventory, { ...target, sourceId: 'ambiguous' }] }));
