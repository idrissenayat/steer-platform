...
import { createScopeSchedulerClient } from '../src/client.ts';

function fixture() {
  const target = { organizationId: 'org', reviewId: randomUUID(), preparationDigest: 'a'.repeat(64) };
  const input = { ...target, expiresAt: new Date(Date.now() + 3600000).toISOString() };
  const id = `steer-scope/v1/org/${target.reviewId}`, run = randomUUID(), config = { namespace: 'default', taskQueue: 'scope' };
  const state = { exists: false, starts: 0, namespaceCalls: 0, retention: 86400, lost: false, historyMissing: false, historyPatch: {} as Record<string,unknown>, payload: target,
    description: { workflowId: id, runId: run, type: 'reviewIntentScope', taskQueue: config.taskQueue, status: { name: 'RUNNING' } } };
  const client = { options: { namespace: config.namespace }, withDeadline: async (_deadline: number, work: () => Promise<unknown>) => work(),
    workflowService: {
      ...
      getWorkflowExecutionHistory: async (request: { maximumPageSize: number }) => {
        ...
        return { history: { events: [{ eventId: 1, workflowExecutionStartedEventAttributes: { workflowType: { name: 'reviewIntentScope' },
          taskQueue: { name: config.taskQueue }, workflowExecutionTimeout: { seconds: 1800 }, input: { payloads: [{ metadata: { encoding: Buffer.from('json/plain') }, data: Buffer.from(JSON.stringify(state.payload)) }] }, ...state.historyPatch } }] } };
      },
    },
    workflow: {
  ...
  const create = () => createScopeSchedulerClient(client, config);
  return { input, state, client, create, id, run, config };
}
test('scope scheduler verifies exact retained start and recovers after reconstruction with no second start', async () => {
...
test('foreign original input or workflow description cannot masquerade as an exact acknowledgement', async () => {
  ...
  const f = fixture(); f.state.exists = true; f.state.payload = { ...f.state.payload, preparationDigest: 'b'.repeat(64) };
  assert.equal((await f.create().start(f.input, async () => {})).outcome, 'unknown'); assert.equal(f.state.starts, 0);
  f.state.payload = { ...f.state.payload, preparationDigest: f.input.preparationDigest }; f.state.historyMissing = true;
  assert.equal((await f.create().start(f.input, async () => {})).outcome, 'unknown'); assert.equal(f.state.starts, 0);
});
...
test('scope scheduling refuses foreign start policy, continuation and malformed retained payloads without starting replacements',async()=>{
  for(const historyPatch of [{retryPolicy:{maximumAttempts:2}},{cronSchedule:'@daily'},{continuedExecutionRunId:randomUUID()},
    {parentWorkflowExecution:{workflowId:'parent',runId:randomUUID()}},{attempt:2},{workflowExecutionTimeout:{seconds:1799}},
    {workflowExecutionTimeout:{seconds:1800,nanos:1}},{input:{payloads:[]}},{input:{payloads:[{metadata:{encoding:Buffer.from('json/plain')},data:Buffer.from([255])}]}},
    {input:{payloads:[{metadata:{encoding:Buffer.from('json/plain')},data:Buffer.alloc(4097)}]}}]){
    const f=fixture();f.state.exists=true;f.state.historyPatch=historyPatch;
    assert.equal((await f.create().start(f.input,async()=>{})).outcome,'unknown');assert.equal(f.state.starts,0);
  }
});
test('scope scheduling keeps fixed namespace, exact input and current authority across every remote wait',async()=>{
  const f=fixture();for(const patch of [{namespace:'foreign'},{taskQueue:'bad\n'},{extra:true}])assert.throws(()=>createScopeSchedulerClient(f.client,{...f.config,...patch}));
  await assert.rejects(f.create().start({...f.input,batchIds:[]} as any,async()=>{}));
  for(const expiresAt of ['',123,null,'2026-09-08'])assert.equal((await f.create().start({...f.input,expiresAt} as any,async()=>{})).outcome,'unavailable');
  assert.equal((await f.create().start(f.input,async()=>true as any)).outcome,'unknown');assert.equal(f.state.starts,0);
  const scheduler=f.create();let checks=0;
  assert.equal((await scheduler.start(f.input,async()=>{if(++checks===3)f.client.options.namespace='foreign';})).outcome,'unknown');assert.equal(f.state.starts,0);
});
