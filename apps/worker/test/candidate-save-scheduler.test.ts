...
function fixture() {
  ...
  const id = `steer-candidate-save/v1/org/${target.operationId}`, run = randomUUID(), config = { namespace: 'default', taskQueue: 'candidate-save' };
  const state = { exists: false, starts: 0, namespaceCalls: 0, retention: 86400, lost: false, historyMissing: false, payload: target,
    startedPatch: {} as Record<string, unknown>,
    description: { workflowId: id, runId: run, type: 'saveCandidateBundle', taskQueue: config.taskQueue, status: { name: 'RUNNING' } } };
  const client = { options: { namespace: config.namespace }, withDeadline: async (_deadline: number, work: () => Promise<unknown>) => work(),
    workflowService: {
      ...
      getWorkflowExecutionHistory: async (request: { maximumPageSize: number }) => {
        ...
        return { history: { events: [{ eventId: 1, workflowExecutionStartedEventAttributes: { workflowType: { name: 'saveCandidateBundle' },
          taskQueue: { name: config.taskQueue }, workflowExecutionTimeout: { seconds: 300 }, input: { payloads: [{ metadata: { encoding: Buffer.from('json/plain') }, data: Buffer.from(JSON.stringify(state.payload)) }] }, ...state.startedPatch } }] } };
      },
    },
    workflow: {
  ...
  const create = () => createCandidateSaveSchedulerClient(client, config);
  return { input, state, client, create, id, run, config };
}
test('candidate-save scheduler verifies exact retained start and recovers after reconstruction with no second start', async () => {
...
test('candidate scheduler rejects retry, cron, parent, continued and timeout-substituted histories without dispatching a replacement', async () => {
  for (const patch of [{ retryPolicy: {} }, { cronSchedule: '* * * * *' }, { parentWorkflowExecution: {} },
    { continuedExecutionRunId: randomUUID() }, { attempt: 2 }, { workflowExecutionTimeout: { seconds: 301 } },
    { workflowExecutionTimeout: { seconds: 300, nanos: 1 } }]) {
    const f = fixture(); f.state.exists = true; f.state.startedPatch = patch;
    assert.equal((await f.create().start(f.input, async () => {})).outcome, 'unknown'); assert.equal(f.state.starts, 0);
  }
});
