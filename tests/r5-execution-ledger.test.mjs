...
test('0098: actual corrected hooks produce exact-ID execution seals while every unmapped case stays uncovered', () => {
  ...
  assert.equal(report.required, 4036); assert.equal(report.executed, 63); assert.equal(report.passed, 63); assert.equal(report.failed, 0); assert.equal(report.uncovered, 3973);
  ...
  assert.equal(report.families.R5.executed, 2); assert.equal(report.families.R5.uncovered, 7);
...
test('0098: CLI report is truthful and complete-coverage mode fails while uncovered cases remain', () => {
  ...
  for (const mode of [[], ['--report'], ['--require-complete']]) {
    ...
    const report = JSON.parse(result.stdout); assert.equal(report.completeCoverage, false); assert.equal(report.uncovered, 3973); assert.equal(report.failed, 0);
  }
