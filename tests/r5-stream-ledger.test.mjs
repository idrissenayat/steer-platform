...
test('0104: quick and strict preflight truthfully retain heavy unexecuted cases and never claim completeness', () => {
  ...
  assert.equal(quick.executed, 307); assert.equal(quick.families.ACCESSIBILITY.executed, 0); assert.equal(quick.families.ACCESSIBILITY.uncovered, 16);
  ...
  assert.equal(strict.completionPreflight.unmappedFullCaseCount, 3713); assert.equal(strict.completionPreflight.unmappedFullIdsDigest, sha256(jcs(missing)));
