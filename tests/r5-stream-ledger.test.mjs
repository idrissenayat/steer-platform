...
test('0104: quick and strict preflight truthfully retain heavy unexecuted cases and never claim completeness', () => {
  ...
  assert.equal(quick.executed, 341); assert.equal(quick.families.ACCESSIBILITY.executed, 0); assert.equal(quick.families.ACCESSIBILITY.uncovered, 16);
  ...
  assert.equal(strict.completionPreflight.unmappedFullCaseCount, 3679); assert.equal(strict.completionPreflight.unmappedFullIdsDigest, sha256(jcs(missing)));
