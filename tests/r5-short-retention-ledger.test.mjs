...
test('0110: fresh execution credits eight boundaries only and retains all independent and live limitations', () => {
  const report = runCorrectedCoverage(); assert.equal(report.passed, 381); assert.equal(report.executed, 381); assert.equal(report.failed, 0); assert.equal(report.uncovered, 3655);
