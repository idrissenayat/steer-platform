...
test('0108: exactly 27 negative IDs have fresh execution seals without claiming class boundaries or formal closure', () => {
  const report = runCorrectedCoverage(); assert.equal(report.executed, 338); assert.equal(report.passed, 338); assert.equal(report.failed, 0); assert.equal(report.uncovered, 3698);
  assert.deepEqual(report.families['LIFECYCLE-GRAPH-NEGATIVE'], { required: 30, executed: 27, passed: 27, failed: 0, uncovered: 3 });
  ...
  for (const row of report.executions.filter((entry) => entry.id.startsWith('LIFECYCLE-GRAPH-NEGATIVE:'))) assert.equal(row.observationCount, 2);
