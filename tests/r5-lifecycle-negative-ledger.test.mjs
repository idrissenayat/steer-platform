...
test('0108: exactly 27 negative IDs have fresh execution seals without claiming class boundaries or formal closure', () => {
  const report = runCorrectedCoverage(); assert.equal(report.executed, 389); assert.equal(report.passed, 389); assert.equal(report.failed, 0); assert.equal(report.uncovered, 3647);
  assert.deepEqual(report.families['LIFECYCLE-GRAPH-NEGATIVE'], { required: 30, executed: 30, passed: 30, failed: 0, uncovered: 0 });
  assert.equal(report.families['LIFECYCLE-GRAPH'].executed, 48); assert.equal(report.families.MIGRATION.executed, 0); assert.equal(report.families.R5.passed, 9);
