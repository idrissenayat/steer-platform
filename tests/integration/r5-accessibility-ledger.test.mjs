...
test('0104 full: all 16 exact original accessibility cases actually execute with a full same-run positive control', () => {
  assert.equal(report.profile, 'full'); assert.equal(report.executed, 397); assert.equal(report.passed, 397); assert.equal(report.failed, 0); assert.equal(report.uncovered, 3639);
...
test('0104 full: saved full report exactly matches a fresh execution, never imported as proof', () => {
  assert.deepEqual(JSON.parse(readFileSync(new URL('../../intent/0113/FULL-EXECUTION-REPORT.json', import.meta.url))), report);
});
