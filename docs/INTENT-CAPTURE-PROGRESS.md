...
| C22 | I5 Save/reopen | Measured provider request load and end-to-end latency satisfy documented human-UX acceptance thresholds on the representative journey. | Pending | [0299](../intent/0299/EVIDENCE.md) verifies only a test-only corpus protocol: 42 distinct files, 34 semantic sources, five queries / 27 simulated provider attempts and 126 policy calls. Synthetic metadata authorities and a head-reader caller stand-in do not prove integrated authorization or records costs. Large documents need 63 attempts. No production change: confirmation remains 7,637. [Next](../intent/0298/REQUEST-BUDGET-PLAN.md): records/history/key/policy and outer-boundary feasibility, coherent integration, full unchanged benchmark. Prior delayed failure and unexplained 0289 recovery remain retained. |
...
| 2026-09-09 / 0299 | 17/25 | 68% | +0 percentage points | Test-only corpus protocol feasibility: all 42 distinct physical files / 34 semantic sources match; five bounded queries and 27 simulated provider attempts retain 126 source-policy calls. Eight focused and 130 shared-fixture tests pass. Large-document diagnostic uses 63 attempts. Metadata authorities remain synthetic; full resolver/records cost and the integrated corpus allocation are unproven. No production change or user-workflow acceptance. Next finish records/history and outer-boundary feasibility before coherent integration. |
...
with 0299 completing only the corpus protocol experiment. Finish the records/history,
key, policy and outer-callback feasibility before production integration. Preserve fresh
