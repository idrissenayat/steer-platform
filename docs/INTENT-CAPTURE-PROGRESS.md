...
| C22 | I5 Save/reopen | Measured provider request load and end-to-end latency satisfy documented human-UX acceptance thresholds on the representative journey. | Pending | [0297](../intent/0297/EVIDENCE.md) shares immutable corpus evidence within each read-only preview, with fresh grants/head/draft and all other verification. Repository-body downloads fall 75% (preview 172 → 43; confirmation 344 → 86), but total requests fall only about 7% to 3,722 / 7,637. Reconstructed/repeated confirmation use 7,609 / 7,607. The authenticated synthetic save/recovery/reopen, 46 focused tests, 40 native save checks, 1,381 package/application tests, 88 prototype tests, types and build pass. Confirmation still performs 7,488 identity-head checks; whole records/result/observation and identity validation remains the bottleneck. The 200-attempt/full-performance/live targets are not passed. Prior delayed failure and unexplained 0289 recovery-unknown remain retained. |
...
0297 removes repeated corpus-body fetching within preview; now consolidate the
whole records/result/observation and identity-validation read set, retaining fresh
authority before content/effects/release and final readback. Body-only work cannot
close a confirmation that still performs 7,488 identity-head checks.
