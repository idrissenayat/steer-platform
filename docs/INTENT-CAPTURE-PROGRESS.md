...
| C22 | I5 Save/reopen | Measured provider request load and end-to-end latency satisfy documented human-UX acceptance thresholds on the representative journey. | Pending | [0305](../intent/0305/EVIDENCE.md) adds the owned metadata-first records reader with independent per-group policy calls and complete final row/lifecycle checks; the native corpus/records portion uses 56 simulated attempts with the existing test crypto/SDK oracle. The actual application factory is unchanged; confirmation is still 7,637. Production codecs/key-policy services and outer effect integration remain before the unchanged full benchmark under [0298](../intent/0298/REQUEST-BUDGET-PLAN.md). Prior failures remain retained. |
...
cryptographic/SDK decoder remains a test oracle. Neither reader is installed in
the actual application factory; complete production codec/key-policy composition
and outer effect phases next, then run the whole-action acceptance benchmark.
