...
| C22 | I5 Save/reopen | Measured provider request load and end-to-end latency satisfy documented human-UX acceptance thresholds on the representative journey. | Pending | [0300](../intent/0300/EVIDENCE.md) verifies the test-only native records portion: 20 encrypted records, six SDK exchanges, 22 simulated provider attempts including OIDC/Git grants, four transactions and full final readback. [0299](../intent/0299/EVIDENCE.md) covers the corpus batch experiment. Neither is installed in the application; per-purpose policies, outer callbacks and effect boundaries remain to integrate. Full confirmation still costs 7,637. [Next](../intent/0298/REQUEST-BUDGET-PLAN.md): combined boundary feasibility, coherent integration, unchanged full benchmark. Prior delayed failure and unexplained 0289 recovery remain retained. |
...
with 0299 covering the corpus experiment and 0300 the native records experiment.
Finish combined policy, key-provider and outer-callback/effect feasibility before production integration. Preserve fresh
