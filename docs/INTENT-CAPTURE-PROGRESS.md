...
| C22 | I5 Save/reopen | Measured provider request load and end-to-end latency satisfy documented human-UX acceptance thresholds on the representative journey. | Pending | [0304](../intent/0304/EVIDENCE.md) connects the native batch adapter to an owned multi-revision corpus collector and verifies it with native records/OIDC/Git checks at 53 simulated attempts for that portion. The actual application factory is unchanged; confirmation is still 7,637. Independent records/key policies, complete lifecycle authority, owned records composition and outer effects remain before the unchanged full benchmark under [0298](../intent/0298/REQUEST-BUDGET-PLAN.md). Prior failures remain retained. |
...
it does not install the collector or the still-test-only records reader in the factory.
