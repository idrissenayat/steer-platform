...
| C22 | I5 Save/reopen | Measured provider request load and end-to-end latency satisfy documented human-UX acceptance thresholds on the representative journey. | Pending | [0284](../intent/0284/EVIDENCE.md) reduced requests. [0285](../intent/0285/EVIDENCE.md) implements the [benchmark prefix](INTENT-CAPTURE-PERFORMANCE.md): draft-read targets pass, but source review exceeds 200 attempts and fails closed. Later-stage, full failure/concurrency and issuer-latency coverage remain open; roughly 50,000 confirmation requests are still unacceptable. |
