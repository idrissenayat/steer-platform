...
| C22 | I5 Save/reopen | Measured provider request load and end-to-end latency satisfy documented human-UX acceptance thresholds on the representative journey. | Pending | [0299](../intent/0299/EVIDENCE.md) verifies a test-only corpus protocol: 42 distinct files / 34 semantic sources, five queries and 126 policy calls. The actual OIDC/Git-resolver follow-up totals 33 cold attempts (seven bootstrap + 26 corpus), 31 next-request and 57 two-phase attempts. Source metadata remains synthetic; HTTP/records and actual persistence boundaries are not integrated. Large-document standalone diagnostic needs 63 attempts. No production change: confirmation remains 7,637. [Next](../intent/0298/REQUEST-BUDGET-PLAN.md): records/history/key/policy and outer-boundary feasibility, coherent integration, full unchanged benchmark. Prior delayed failure and unexplained 0289 recovery remain retained. |
