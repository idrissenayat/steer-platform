...
4. Older-target proposals can be deliberately selected for verification. The
   chooser displays their original target and the current review commit separately.
   **0267** resolves the mismatch by requiring [unchanged-target continuity](EXISTING-CANDIDATE-DESTINATION.md)
   and additional current policy eligibility. Listing or selection proves neither.
   The system never silently rebases or clears the selection to create another
   proposal. Changed/unavailable targets block continuation; a separately chosen
   new amendment is a different human direction, not a retry.
