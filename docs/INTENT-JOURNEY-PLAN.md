...
Latest checkpoint: **0248 binds the recorded assessment to direction/drafting**.
The actual editor requires current complete findings before a human chooses how to
develop non-empty reviewed scope. A separate assessed server factory independently
reads verified findings, stores them with the exact direction and revalidates on
original restoration/start/worker request. Both roles receive that context without
an earlier Exam. Explicitly complete empty inventory needs no scope model call.
See [0248 evidence](../intent/0248/EVIDENCE.md).
...
Next prioritize larger-corpus generation context and semantic evaluation alongside
real authority and actual human save/reopen. Expired/superseded scope originals need
