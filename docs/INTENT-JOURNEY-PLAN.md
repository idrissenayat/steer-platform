...
Latest checkpoint: **0242 connects actual scope-batch SQL ownership and encrypted
exchanges to the pinned recorded SDK**, with acknowledged dispatch and a newly
stored request required before transport. Completed recovery verifies the original
response without another reservation/call or dispatch credentials. Unknown work
does not resend; current authority, human corrections, cancellation and late
dependencies remain guarded. See [0242 evidence](../intent/0242/EVIDENCE.md).
0241's authenticated query, combined coverage reader and the existing editor/draft
work remain in place. The runner and reader are uninstalled by default; actual
editor binding, scope preparation/reference-only Temporal, expired observation
access and real authority remain unconnected. Legacy generation gates are unchanged.
Next prioritize reference-only scope execution and source preparation into the
actual editor, real authority, larger-corpus context and semantic evaluation.
