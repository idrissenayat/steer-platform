...
Latest checkpoint: **0247 adds retained scope discovery and read-first recovery**
to 0246's actual editor: find bounded references for the exact latest saved draft,
read verified findings before any start, and explicitly recover only the same
pending workflow. Discovery is metadata only, not newest-first or a duplicate
verdict; different corpus versions cannot become current findings.
See [0247 evidence](../intent/0247/EVIDENCE.md).
...
this is not live signed-in acceptance. Scope discovery also remains uninstalled.
Next prioritize server-bound assessment consumption for
