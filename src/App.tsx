...
function IntentDetailPanel({ allIntents, displayedRevision, inFlightCount, intent, onAction, onBack, onClose, onExternalExit, onNavigate }: {
  ...
  return (
    <DrawerFrame label={`${intent.title} intent detail`} onClose={onClose} wide>
      ...
      <section className="review-panel__section" data-detail-section="domains"><p className="section-label">Domain tags and review route</p><div className="tag-row">{intent.domainTags.map((domain) => <span className="domain-tag" key={domain}>{domain}</span>)}</div><p className="detail-route">{intent.domainTags.includes("accessibility") ? "Accessibility is active: an independent agent review is required; human review appears only for an escalation or manual release validation." : "Default-open route: domain assurance activates only when a tagged risk requires it."}</p></section>
  ...
  return (
    <div className="app-shell">
      ...
      <aside aria-hidden={modalOpen ? true : undefined} className="sidebar">
        ...
        <div className="workspace-switcher"><span className="workspace-avatar" aria-hidden="true">SP</span><div><strong>STEER Platform</strong><span>Pilot workspace · v3.2</span></div><b aria-hidden="true">⌄</b></div>
        ...
        <div className="main-content">
          ...
          <section className="organization-section" id="organization">
            ...
            <div className="organization-grid">
              ...
              <article><span>Domain assurance</span><strong>{organizationSetup.domainReviewAgents.length} independent reviewers</strong><p>One consolidated exception brief · human specialists only on trigger</p></article>
